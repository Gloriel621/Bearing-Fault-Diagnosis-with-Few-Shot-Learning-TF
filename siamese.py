...
class Siamese_Loader:
    ...
    def __init__(self, X_train, y_train, X_val, y_val):
        ...
        self.indices = {
            "train": [np.where(y_train == i)[0] for i in self.classes["train"]],
            "val": [np.where(y_val == i)[0] for i in self.classes["val"]],
        }

        print(self.classes)
        print(
            [len(c) for c in self.indices["train"]],
            [len(c) for c in self.indices["val"]],
        )
        print("t2")
    ...
    def make_oneshot_task(self, N, s="val", language=None):
        ...
        if N > n_classes:
            raise ValueError(
                "{} way task has greter than {} classes".format(N, n_classes)
            )
        ...
        test_image = np.asarray([X[X_indices[true_category][ex1]]] * N).reshape(
            N, w, h, 1
        )
        ...
        for idx, category in enumerate(categories[1:]):
            n_examples = len(X_indices[category])
            support_set[idx + 1, :, :] = X[
                X_indices[category][rng.randint(0, n_examples)]
            ]
        support_set = support_set.reshape(N, w, h, 1)
        targets = np.zeros((N,))
        targets[0] = 1
        targets, test_image, support_set, categories = shuffle(
            targets, test_image, support_set, categories
        )
    ...
    def make_oneshot_task2(self, idx, s="val"):
        ...
        for index in range(N):
            support_set[index, :, :] = X_train[
                rng.choice(indices_train[index], size=(1,), replace=False)
            ]
        support_set = support_set.reshape(N, w, h, 1)
    ...
    def test_oneshot2(self, model, N, k, s="val", verbose=0):
        ...
        err_print_num = 0
        for idx in range(k):
            inputs, targets, categories = self.make_oneshot_task2(
                idx, s
            )  # return pairs, targets, categories
            n_classes, w, h, _ = inputs[0].shape
            inputs[0] = inputs[0].reshape(n_classes, w, h)
            ...
            preds.append(
                [categories[np.argmax(targets)], categories[np.argmax(probs)]]
            )  # preds.append (1,1), (3,6)~~~~~~
...
def train_and_test_oneshot(settings, siamese_net, siamese_loader: Siamese_Loader):
    settings["best"] = -1
    settings["n"] = 0
    print(settings)
    ...
    for i in range(settings["n"], settings["n_iter"]):
        ...
        if i % settings["evaluate_every"] == 0:
            val_acc, preds, prods = siamese_loader.test_oneshot2(
                siamese_net, settings["N_way"], settings["n_val"], verbose=False
            )

            preds = np.array(preds)
            print("settings['best']", settings["best"])
            print("val_acc", val_acc)
            if val_acc >= settings["best"]:
                print(
                    "\niteration {} evaluating: {}".format(i, val_acc)
                )  ##iteration X evaluating x
