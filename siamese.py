...
class Siamese_Loader:
    ...
    def test_oneshot2(self,model,N,k,s="val",verbose=0):
        ...
        err_print_num = 0
        for idx in range(k):            
            inputs, targets,categories = self.make_oneshot_task2(idx,s) # return pairs, targets, categories
            n_classes, w, h,_ = inputs[0].shape
            inputs[0]=inputs[0].reshape(n_classes,w,h)
            inputs[1]=inputs[1].reshape(n_classes,w,h) 
            probs = model.predict(inputs)
            ...
            preds.append([categories[np.argmax(targets)],categories[np.argmax(probs)]])  # preds.append (1,1), (3,6)~~~~~~
...
def train_and_test_oneshot(settings,siamese_net,siamese_loader: Siamese_Loader):
    ...
    for i in range(settings['n'], settings['n_iter']):
        ...
        if i % settings['evaluate_every'] == 0:
            val_acc,preds, prods = siamese_loader.test_oneshot2(siamese_net,settings['N_way'],settings['n_val'],verbose=False)

            preds = np.array(preds)
            print("settings['best']", settings['best'])
            print("val_acc", val_acc)
            if val_acc >= settings['best'] :
                print("\niteration {} evaluating: {}".format(i,val_acc))  ##iteration X evaluating x
